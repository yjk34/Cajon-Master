...
final static int DRAW_TRACE_MAGIC_NUM = 100;
...
final static String[] fillinList =    {"310201121",
									   "312212212",
									   "312012022",
									   "41001210110012122",
									   "41201201201202222",
...
void initScene() {
	size(WINDOW_WIDTH * 2, WINDOW_HEIGHT, P3D);
	background(255);
	noStroke();
	//colorMode(RGB, 100);
...
void initGlobalVariables() {
	mouseTraceBegin = new Coord(0, 0);
	mouseTraceEnd = new Coord(0, 0);
...
void drawMarkerTrace() {
	fill(255, 255, 0);
	for (int i=0; i<pntsList.size(); i++) {
		ellipse(WINDOW_WIDTH + DRAW_TRACE_MAGIC_NUM + (float)pntsList.get(i).x, (float)pntsList.get(i).y, 5, 5);
	}
}